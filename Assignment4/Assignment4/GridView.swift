...
@IBDesignable class GridView : UIView {
    ...
    //var grid : Grid = Grid(20, 20, cellInitializer: Grid.gliderInitializer)
    var grid : GridProtocol? {
        didSet {
            self.size = (grid?.size.rows)!
        }
    }
    ...
    private func processTouch(_ touches: Set<UITouch>, with event: UIEvent?) {
        if let touch = touches.first {
            ...
            if self.grid  != nil {
                var g = self.grid!
                g[x, y] = g[x, y].toggle(value: (g[x, y]))
                delegate?.viewDidUpdate(withGrid: g)
                let nc = NotificationCenter.default
                let name = Notification.Name(rawValue: "ViweUpdate")
                let n = Notification(name: name, object: g, userInfo: ["GridView" : self])
                nc.post(n)
                self.grid = g
            }
        }
